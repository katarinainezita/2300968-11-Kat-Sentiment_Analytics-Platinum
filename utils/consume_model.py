...
async def get_sentiment_result(input_text, model_type):
    if model_type == "rnn" or model_type == "lstm" or model_type == "lstm fine tuned" or model_type == "rnn fine tuned" or model_type == "nn fine tuned":
        ...
        if model_type == "rnn":
            ...
            model = load_model('models/model_lstm_finetuned.h5')
        elif model_type == "rnn fine tuned":
            model = load_model('models/model_rnn_fine.h5')
        elif model_type == "nn fine tuned":
            df = pd.read_csv("utils/data/sentiment_data.csv")
            data_preprocessed = df.text.tolist()

            loaded_model = pickle.load(open("models/model_nn_fine.p", "rb"))
        ...
        loaded_model = pickle.load(open("models/model_nn.p", "rb"))
